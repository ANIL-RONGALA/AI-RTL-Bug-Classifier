...
AI-RTL-Bug-Classifier/
│ ├── dataset/ # 300 made RTL bug samples
│ ├── sample_001/│    │ ├── code.v   │ ├── log.txt │ └── label.txt
│ ├── src/
│ ├── dataset_generator.py # Generator for synthetic datasets
│ ├── train.py # Training script (TF-IDF + Logistic Regression) and predict.py Make predictions on new code or logs
│ ├── utils.py # Loading the dataset and label maps
│ ├── model.pkl # Saved model (made after training) │ ├── vectorizer.pkl ├── TF-IDF vectorizer (made)
│ └── class_map.json # id → label mapping (made)
│ ├── notebooks/
│ └── AI_EDA_RTL_Bug_Classifier.ipynb # Notebook for experiments
